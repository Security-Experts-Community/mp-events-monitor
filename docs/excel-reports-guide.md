...
При статусе audit необходимо проанализировать поле "audit_status". В данном случае значение Obsolete указывает на то, что сканирование устарело. Требуется найти задачу аудита и проверить почему сканирование не происходит своевременно. Все статусы поля audit_status перечислены выше.
