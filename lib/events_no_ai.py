...
class EventsWorker:
        ...
        if self.policies.rebuilt_policies:
            ...
            for index, policy in enumerate(self.policies.rebuilt_policies):
                ...
                if asset_ids:
                    ...
                    temp_time_from = time_from_value
                result = self.take_events(group_id, temp_time_from, filter_new, out_folder, policy)
                if "host_ids" not in self.policies.rebuilt_policies[index].keys():
                    self.policies.rebuilt_policies[index].update({"host_ids": result})
                else:
                    for host in result:
                        self.policies.rebuilt_policies[index]["host_ids"].update({host: result[host]})
    ...
    def take_events(self, group_id, time_from, event_filter, out_dir, all_policy):
        ...
        while try_number < self.settings.reconnect_times:
            try:
                try_number += 1
                response_temp = requests.session().post(url=url, json=data, headers=self.auth.headers, verify=False,
                                                        cookies=self.auth.cookies, params=param)
                ...
                if response_temp.status_code == 200:
                    ...
                    self.logger.warning(f"Response code: {response_temp.status_code}. Try number: {try_number}. Next try after 5 seconds")
                    time.sleep(5)
                elif response_temp.status_code == 400:
                    response = response_temp.json()
                    resp_mes = response["message"]
                    self.logger.error(f"Response code: {response_temp.status_code}. Response message: {resp_mes}.")
